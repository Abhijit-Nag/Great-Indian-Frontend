...
const AddressCartOrder = ({ setIsAddressSet, setOpenAddressModal, products }) => {
    ...
    const callPayment = async () => {
        ...
        for (let i = 0; i < products.length; i++) {
            amount = amount + (products[i].price.cost * products[i].quantity);
        }
        ...
        const options = {
            ...
            // callback_url: `http://localhost:8000/api/cartorder/verification/${userId}/${amount}`,
            callback_url: `${process.env.REACT_APP_SERVER_API_KEY}/api/cartorder/verification/${userId}/${amount}`,
            prefill: {
                name: name,
...
export default AddressCartOrder;
